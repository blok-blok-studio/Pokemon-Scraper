---
name: voice-outreach
description: Make AI voice calls to card shops and pawn shops to ask about Pokemon card inventory. Uses Bland.ai for voice calls. Only call during business hours in the target's timezone.
metadata:
  openclaw:
    requires:
      bins: ["node"]
      env: ["BLAND_API_KEY", "ANTHROPIC_API_KEY"]
...
## Description
AI-powered voice calls to card shops and pawn shops asking about Pokemon card inventory. Calls are made via Bland.ai, transcripts are analyzed by Claude for follow-up signals.
...
- If Bland API fails, skip that call and log — never retry a failed call immediately
